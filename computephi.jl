function computephi(dim,compute,prior_type,gamma,ilambda,rtol,x,ncoord,n,h_node,unstructured)
  ...
  while (norm(R)>rtol)
    ...
    if (abs(1/cond(J,1)))<1e-8
      print("\n")
      println("Newton Failed, near to singular matrix")
    end
    dlam=(-R'/J)'
    lambda=lambda+dlam
    niter=niter+1
    if (niter>100)
      print("\n")
      println("Newton Failed 2, no convergence in 100 iterations")
    end
  end
  lambda=lambda-dlam
  print("\n")
  println("Newton interations: $niter")

  ###########################################################################
  # compute phider in an matrix nx2 as follows:
  #       phider = [ ... ... ; der_x  der_y; ... ...]
  #
  ...
  #
  # this is arranged to compute phider in the format
  #              [ ... ... ; der_x  der_y; ... ...]
  ###########################################################################
  ...
  elseif (compute == 2)
    ...
    if (dim == 1)
      invH=1/H[1,1];
      phider=phi.*(xc_c*((invH-invH*A)[1]))+phi.*MA-phi*MC
    elseif (dim == 2)
      invH=inv(H);
  ...
  return phi,phider,contribute,len,lambda
